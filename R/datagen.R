...
#' - \eqn{M_{i,k} = \alpha_k X_i + e_{1_{i,k}}}, where \eqn{e_{1_{i,k}} \stackrel{i.i.d}{\sim} N(0, 1), k = 1, ..., p}, and \eqn{cov(e_{1_{i,k}}, e_{1_{i,j}}) = \rho} for k not equal j
#' - \eqn{Y_i = X_i + \sum_k b_{k} M_{i,k} + e_{2_{i}}}, where \eqn{e_{2_{i}} \stackrel{i.i.d}{\sim} N(0, 1)}
#' @description The 6th-15th M variables (i.e., M6,M7,...,M14,M15) are set to be the true mediators (i.e., having non-zero alpha and beta coefficients).
...
#' @param a effect size of X-M path (i.e., non-zero alpha coefficient)
#' @param b effect size of M-Y path (i.e., non-zero beta coefficient)
#' @param rho effect size of the correlation structure of M
#'
